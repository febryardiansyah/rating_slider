...
typedef IconBuilder = Widget Function(BuildContext context, int index, RatingType type);
...
class _RatingSliderState extends State<RatingSlider> {
  ...
  void _updateRatingFromPosition(Offset localPosition) {
    final boxWidth = widget.areaItemSize * widget.starCount;
    ...
    setState(() {
      _rating = finalRating.clamp(0.0, widget.starCount.toDouble());
    });
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      onPanUpdate: (details) => _updateRatingFromPosition(details.localPosition),
      child: Row(
